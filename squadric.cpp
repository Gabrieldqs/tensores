...
void gcgSUPERQUADRIC::create(unsigned int slicesphi, unsigned int slicestheta,FLOAT xScale, FLOAT yScale, FLOAT zScale){
    ...
    pointPos = 0;
    if(notcreated) pontos = (point*) malloc(((slicesphi+slicesphi-2) * (slicestheta + 1) + slicestheta+slicestheta+2) * sizeof(point));
    this->xScale = xScale;
    this->yScale = yScale;
    this->zScale = zScale;
...
void gcgSUPERQUADRIC::draw(bool drawMesh, bool drawNormals, bool invert){
  ...
  for(unsigned int i = 0; i < pointPos; i++){
//    if(invert) glNormal3f(-pontos[i].nx,-pontos[i].ny,-pontos[i].nz);
//    else glNormal3f(pontos[i].nx,pontos[i].ny,pontos[i].nz);
