...
void sqSolidEllipsoid ( SuperQuadric *sq, int make_display_list, int gen_texture_coordinates ) {
	...
	for ( Y = 0; Y < sq->u_segs; Y++ ) {
		...
		for ( X = 0; X < sq->v_segs; X++ ) {
			/* VERTEX #1 */
			sqEllipsoid ( sq->a1, sq->a2, sq->a3, U, V, sq->n, sq->e, &x, &y, &z, &nx, &ny, &nz );
			glNormal3f ( nx, ny, nz );
			glTexCoord2f ( S, T );
			glVertex3f ( x, y, z );

			/* VERTEX #2 */     
			sqEllipsoid ( sq->a1, sq->a2, sq->a3, U + dU, V, sq->n, sq->e, &x, &y, &z, &nx, &ny, &nz );
			glNormal3f ( nx, ny, nz );
			glTexCoord2f ( S + dS, T );
			glVertex3f ( x, y, z );

			/* VERTEX #3 */
			sqEllipsoid (sq->a1, sq->a2, sq->a3, U + dU, V + dV, sq->n, sq->e, &x, &y, &z, &nx, &ny, &nz );
			glNormal3f ( nx, ny, nz );
			glTexCoord2f ( S + dS, T + dT );
			glVertex3f ( x, y, z );

			/* VERTEX #4 */
			sqEllipsoid ( sq->a1, sq->a2, sq->a3, U, V + dV, sq->n, sq->e, &x, &y, &z, &nx, &ny, &nz );
			glNormal3f ( nx, ny, nz );
			glTexCoord2f ( S, T + dT );
			glVertex3f ( x, y, z );
